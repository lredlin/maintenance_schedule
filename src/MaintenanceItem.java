...
public class MaintenanceItem implements Item {
	...
	public MaintenanceItem(Periodicity periodicity, int index, Date issueDate, Date completedDate, Date dueDate,
			Owner owner, int specialPeriodicity) {
	...
	public void scheduleItem(Date issueDate, Worker worker) {
		...
		case SPECIAL:
			cal.setTime(issueDate);
			cal.add(Calendar.DAY_OF_WEEK, getSpecialPeriodicity()); // adds plus specialPeriodicity set by Owner in # of
																	// Days
			dueDate = cal.getTime();
			break;
		case WEEKLY:
